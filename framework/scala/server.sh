...
start_server_fg()
{
    ...
    set -x
    scala -cp $classpath $server_class -p $port
}

test_server()
{
    curl -s --connect-timeout 2 "$prefix/soda/scala/echo?a=x" >/dev/null 2>&1
}
...
case $cmd in
    start)
        ...
        ;;
    stop)
        curl -s --connect-timeout 2 "$prefix/soda/scala/stop" && echo
        ;;
    start-fg)
        start_server_fg
        ;;
